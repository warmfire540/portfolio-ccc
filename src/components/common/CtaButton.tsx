...
const CtaButton: React.FC<CtaButtonProps> = ({ text, onClick, primary = true, className = '' }) => {
  const baseClasses =
  ...
  const combinedClasses = `${baseClasses} ${primary ? primaryClasses : secondaryClasses} ${className || defaultSizeClasses}`;

  return (
    <button onClick={onClick} className={combinedClasses} data-testid="cta-button">
      {text}
    </button>
  );
};
